...
/datum/status_effect/buff/eoran_balm_effect/tick()
	...
	if(M.urine_volume < URINE_VOLUME_NORMAL)
		M.urine_volume = min(M.urine_volume + healing_power * 2, URINE_VOLUME_NORMAL)
	...
	if(M.getBruteLoss() < 50 && M.getFireLoss() < 50 && M.getToxLoss() < 50 && M.getOxyLoss() < 50 && M.urine_volume >= URINE_VOLUME_SAFE)
...
/datum/status_effect/debuff/pomegranate_aura/tick()
	...
	if(owner.stat == DEAD)
		owner.urine_volume = max(10, owner.urine_volume - 10)
