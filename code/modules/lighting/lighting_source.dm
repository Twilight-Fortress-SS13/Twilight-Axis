...
#define SETUP_CORNERS_CACHE(lighting_source)                                                               \
	...
	if(has_z_neighbors(source_turf)) {                                                                     \
	...
	var/_multiz_offset = CEILING(lighting_source.light_range, 1) + 1;                                      \
...
/// Whether this z-level has an above or below z-level connected to it. Used to decide if we need to bother generating a 3D (multiz) falloff sheet.
/datum/light_source/proc/has_z_neighbors(turf/T)
	return !!(GET_TURF_ABOVE(T) || GET_TURF_BELOW(T))
...
/datum/light_source/proc/generate_sheet_multiz(range, visual_offset, x_offset, y_offset, center_dir, angle, height)
	var/list/encode = list()
	// We don't have simultaneous multi-z plane rendering, so bound the z depth of the sheet by the light's own range -
	// light can't meaningfully reach further in z than it does in x/y anyway, the falloff formula would zero it out regardless.
	var/z_range = CEILING(range, 1)
