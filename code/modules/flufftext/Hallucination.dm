		...
		if("ranged")
			...
			for(var/i in 1 to rand(5, 8))
				...
				if(prob(50))
					...
					hits++
					target.playsound_local(target, 'sound/misc/piss (1).ogg', 90, 1)
				else
					addtimer(CALLBACK(target, TYPE_PROC_REF(/mob, playsound_local), target, 'sound/combat/hits/hi_arrow3.ogg', 65, 1), rand(5,10))
					target.playsound_local(target, 'sound/misc/piss (2).ogg', 90, 1)
				sleep(rand(CLICK_CD_RANGE, CLICK_CD_RANGE + 6))
				if(hits >= 3 && prob(50))
...
/datum/hallucination/weird_sounds/New(mob/living/carbon/C, forced = TRUE, sound_type)
	...
	var/turf/source = random_far_turf()
	if(!sound_type)
		sound_type = pick("piss","whispers","whispers2","zizo","evil","drill","laughter","creepy")
	feedback_details += "Type: [sound_type]"
	//Strange audio
	switch(sound_type)
		if("piss")
			target.playsound_local(target, 'sound/misc/piss (1).ogg', 80)
			sleep(25)
			target.playsound_local(target, 'sound/misc/piss (2).ogg', 85)
			sleep(25)
			target.playsound_local(target, 'sound/misc/piss (3).ogg', 90)
			sleep(25)
			target.playsound_local(target, 'sound/misc/piss (1).ogg', 100)
		if("whispers")
...
/datum/hallucination/fake_alert/New(mob/living/carbon/C, forced = TRUE, specific, duration = 150)
	set waitfor = FALSE
	..()
	var/alert_type = pick("piss","netted","surrender","nutrition","thirsty","fire")
	if(specific)
		alert_type = specific
	feedback_details += "Type: [alert_type]"
	switch(alert_type)
		if("piss")
			/*target.throw_alert(alert_type, /atom/movable/screen/alert/status_effect/debuff/pissingt1, override = TRUE)
			sleep(50)
			target.throw_alert(alert_type, /atom/movable/screen/alert/status_effect/debuff/pissingt2, override = TRUE)
			sleep(50)*/
			target.throw_alert(alert_type, /atom/movable/screen/alert/status_effect/debuff/pissingt3, override = TRUE)
			target.playsound_local(target, 'sound/misc/piss (1).ogg', 80)
			sleep(5)
			target.playsound_local(target, 'sound/misc/piss (2).ogg', 85)
			sleep(5)
