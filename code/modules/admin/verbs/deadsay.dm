/client/proc/dsay(msg as text)
	...
	if(prefs.muted & MUTE_DEADCHAT)
		to_chat(src, span_danger("I cannot send DSAY messages (muted)."))
		return
	...
	for (var/mob/M in GLOB.player_list)
		if(isnewplayer(M))
			continue
		if (M.stat == DEAD || (M.client.holder && (M.client.prefs.chat_toggles & CHAT_DSAY))) //admins can toggle deadchat on and off. This is a proc in admin.dm and is only give to Administrators and above
			to_chat(M, rendered)
