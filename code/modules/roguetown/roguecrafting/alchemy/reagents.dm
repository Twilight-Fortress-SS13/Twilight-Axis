...
/datum/reagent/medicine/fluroguetest_cure
	name = "Fluroguetest Cure"
	description = "A sterile tonic that helps the body purge the plague."
	reagent_state = LIQUID
	color = "#7cbf6f"
	taste_description = "bitter herbs"
	scent_description = "clean rain"
	metabolization_rate = 0.1 * REAGENTS_METABOLISM

/datum/reagent/medicine/fluroguetest_cure/on_mob_life(mob/living/carbon/M)
	if(volume > 0.99)
		for(var/thing in M.diseases)
			var/datum/disease/D = thing
			if(istype(D, /datum/disease/fluroguetest) && prob(25))
				D.cure()
