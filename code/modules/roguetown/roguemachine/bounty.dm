...
/obj/structure/chair/arrestchair/attack_right(mob/living/carbon/human/A)
	...
	if(!submission)
		if(M.Adjacent(src))
			...
			M.emote("superagony")
			M.visible_message(span_warningbig("The CASTIFICO begins GRUESOMELY SHAVING OUT every bit and ounce of [M]'s FLESH on their HEAD, leaving only a grinning, pissing skull behind. Justice has been served."))
