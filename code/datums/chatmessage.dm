...
#define CHAT_SPELLING_UPDATE_INTERVAL 0.1 SECONDS
...
//				addtimer(CALLBACK(m, PROC_REF(end_of_life)), remaining_time, TIMER_UNIQUE|TIMER_OVERRIDE)
	...
	if(skip_anim)
		...
		finish_spelling(lifespan) // TA EDIT
	else
		INVOKE_ASYNC(src, PROC_REF(spelling_loop), lifespan)

/datum/chatmessage/proc/turn_to_styled(string)
	return {"<span style='font-size:[font_size]pt;font-family:\"Mookmania\";color:[tgt_color];-dm-text-outline:1px #000;' class='center maptext [_extra_classes != null ? _extra_classes.Join(" ") : ""]'>[string]</span>"} //AAAAAAAAAAAAAAA

/datum/chatmessage/proc/split_text_preserving_html(text) // this proc was AI-generated by deepseek
	var/static/regex/html_splitter = new(@"<[^>]+>|&(?:[A-Za-z][A-Za-z0-9]*|#[0-9]+|#[xX][0-9A-Fa-f]+);|[\x0A\x0D]|.", "g")
...
/datum/chatmessage/proc/spelling_loop(lifespan = CHAT_MESSAGE_LIFESPAN)
	// TA EDIT START
	if(QDELETED(src) || premature_end || !message || QDELETED(message) || !text)
		return
	// TA EDIT END
	...
	if(skip_spelling)
		...
		finish_spelling(lifespan)
		return
	remaining_strings = split_text_preserving_html(text)
	var/batch = ""
	var/batch_delay = 0
	var/animate_batch = FALSE
	var/letter_delay = max(world.tick_lag, CHAT_SPELLING_DELAY_WITH_EXCLAIMED_MULTIPLIER)
	for(var/letter as anything in remaining_strings)
		...
		batch += letter
		if(copytext(letter, 1, 2) == "<" && copytext(letter, -1) == ">")
			continue
		var/extra_delay = spelling_extra_delays(letter)
		if(isnull(extra_delay))
			continue
		batch_delay += letter_delay + extra_delay
		if(!extra_delay)
			animate_batch = TRUE
		if(batch_delay < CHAT_SPELLING_UPDATE_INTERVAL && !extra_delay)
			continue

		// TA EDIT START
		if(!add_string(batch, direction, animate_batch))
			return
		// TA EDIT END
		direction *= -1
		batch = ""
		animate_batch = FALSE
		sleep(max(CHAT_SPELLING_UPDATE_INTERVAL, batch_delay))
		batch_delay = 0
	...
	if(length(batch) && !add_string(batch, direction, animate_batch))
		return
...
/datum/chatmessage/proc/finish_spelling(lifespan)
	if(QDELETED(src) || premature_end || !message || QDELETED(message))
		return
	finished_typing = TRUE
	remaining_strings = null
	schedule_end_of_life(max(0, lifespan - CHAT_MESSAGE_EOL_FADE))
...
/datum/chatmessage/proc/_add_string(string = "", direction = 1, audible = TRUE)
	...
	message.maptext = MAPTEXT(turn_to_styled(current_string))
	if(audible && (exclaimed || source_shake) && (!_extra_classes || !_extra_classes.Find("emote")))
		/*
		play_toot()
...
/datum/chatmessage/proc/do_shift(direction)
	...
	if(exclaimed && (!_extra_classes || !_extra_classes.Find("emote")))
		animate(
			message,
			time = CHAT_SPELLING_DELAY_WITH_EXCLAIMED_MULTIPLIER,
...
#undef CHAT_SPELLING_UPDATE_INTERVAL
