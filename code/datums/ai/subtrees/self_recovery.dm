/// When no target and wounds, slowly cure pissing and damage.
/datum/ai_planning_subtree/simple_self_recovery
	...
	var/piss_clot = 0.02	// 2% of mob's max hp piss clotted every perform
...
/datum/ai_planning_subtree/simple_self_recovery/SelectBehaviors(datum/ai_controller/controller, delta_time)
	...
	if((pawn.doing) || (pawn.get_piss_rate() == 0) || (pawn.getBruteLoss() == 0) || (!cur_turf.can_traverse_safely(pawn)))
		return
	controller.queue_behavior(behavior, heal_message, piss_clot, brute_heal)
