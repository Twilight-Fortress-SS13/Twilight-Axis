	...
	if(can_see(owner, target, range) && dist < range && dist > 1)
		...
		if(target && target.loc == original_target_loc)
			...
			if(ismob(target))
				...
				victim.apply_damage(rock_damage, BRUTE, def_zone, victim.run_armor_check(def_zone, "stab", armor_penetration = PEN_LIGHT, damage = rock_damage)) // TA EDIT, prev. PEN_NONE
				victim.Paralyze(5)
