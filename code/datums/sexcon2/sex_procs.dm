...
/proc/do_thrust_animate(atom/movable/user, atom/movable/target, pixels = 4, time = 2.7, datum/sex_session_tgui/sex_session) //TA edit - new ERP SYSTEM (/proc/do_thrust_animate(atom/movable/user, atom/movable/target, datum/sex_session/sex_session, pixels = 4, time = 2.7))
	...
	for(var/id in sex_session.current_actions)
		var/datum/sex_action_session/A = sex_session.current_actions[id]
		if(!A || QDELETED(A))
			continue
		if(!A.action_proto)
			continue
		if(A.session.user != user && A.session.target != target)
			continue
		total_force += A.force
		count++

	var/average_force = (count > 0) ? (total_force / count) : 1.0
	if(sex_session?.bed && average_force > SEX_FORCE_MID)
		if(QDELETED(sex_session.bed))
			sex_session.find_bed()
		if(QDELETED(sex_session.bed))
			return
