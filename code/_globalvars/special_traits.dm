...
/proc/check_trait_incompatibilities(mob/living/carbon/human/H)
	...
	if(advclass.tempo_capable && H.advjob != "Knight Banneret" && H.mind.assigned_role != "Court Agent" && H.mind.assigned_role != "Adventurer" && H.mind.assigned_role != "Prince" && H.mind.assigned_role != "Court Magician" && H.mind.assigned_role != "Inquisitor") // (Easier to filter these out than apply the bool to every subclass)
