	...
	switch(classchoice)
		if("Vanguard")
			...
			if(!SEND_SIGNAL(back, COMSIG_TRY_STORAGE_INSERT, hook, null, TRUE, TRUE))
				addtimer(CALLBACK(src, PROC_REF(move_storage), hook, H.loc), 3 SECONDS)
		...
		if("Runed Volf")
			...
			switch(ammochoice)
				if("Holy Fyrepowder")
					...
					if(!SEND_SIGNAL(belt, COMSIG_TRY_STORAGE_INSERT, powderflask, null, TRUE, TRUE))
						addtimer(CALLBACK(src, PROC_REF(move_storage), powderflask, H.loc), 3 SECONDS)
				if("Thunderpowder")
			...
			if(!SEND_SIGNAL(belt, COMSIG_TRY_STORAGE_INSERT, lockpickring, null, TRUE, TRUE))
				addtimer(CALLBACK(src, PROC_REF(move_storage), lockpickring, H.loc), 3 SECONDS)
			...
			if(!SEND_SIGNAL(belt, COMSIG_TRY_STORAGE_INSERT, garrote, null, TRUE, TRUE))
				addtimer(CALLBACK(src, PROC_REF(move_storage), garrote, H.loc), 3 SECONDS)
