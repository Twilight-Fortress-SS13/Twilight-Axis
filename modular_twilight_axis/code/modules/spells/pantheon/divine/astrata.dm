...
/obj/effect/proc_holder/spell/invoked/projectile/TAsacred_flame
	name = "Sacred Flame"
...
/obj/projectile/magic/TAsacred_flame/on_hit(target, mob/user)
	if(!ismob(target))
		return FALSE
...
/obj/effect/proc_holder/spell/invoked/TArevive/start_recharge()
...
/obj/effect/proc_holder/spell/invoked/TArevive/cast(list/targets, mob/living/user)
	..()
...
/datum/status_effect/debuff/bad_revive
	id = "bad_revive"
	alert_type = /atom/movable/screen/alert/status_effect/debuff/bad_revive
	duration = 2 MINUTES

/atom/movable/screen/alert/status_effect/debuff/bad_revive
	name = "Sun Smite"
	desc = "<span class='warning'>ASTRATA SMITE ME!</span>\n"
	icon_state = "gallows"
...
/datum/status_effect/debuff/bad_revive/on_remove()
...
/obj/effect/proc_holder/spell/invoked/TAastrataspark
	name = "Flame Order"
...
/obj/effect/proc_holder/spell/invoked/TAastrataspark/cast(list/targets, mob/user = usr)
...
/obj/effect/proc_holder/spell/self/TAastrata_gaze/cast(list/targets, mob/user)
	if(!ishuman(user))
		revert_cast()
		return FALSE
	var/mob/living/carbon/human/H = user
	var/skill_level = H.get_skill_level(associated_skill)
	H.apply_status_effect(/datum/status_effect/buff/TAastrata_gaze, skill_level)
	return TRUE
...
/datum/status_effect/buff/TAastrata_gaze
	id = "astratagaze"
	alert_type = /atom/movable/screen/alert/status_effect/buff/TAastrata_gaze
	duration = 20 SECONDS
	var/skill_level = 0
	status_type = STATUS_EFFECT_REPLACE

/datum/status_effect/buff/TAastrata_gaze/on_creation(mob/living/new_owner, slevel)
...
/datum/status_effect/buff/TAastrata_gaze/on_remove()
	. = ..()
	if(ishuman(owner))
		var/mob/living/carbon/human/H = owner
...
/obj/effect/proc_holder/spell/self/TAastrata_fireresist/cast(mob/living/carbon/human/user)
	...
	if(user.has_status_effect(/datum/status_effect/buff/dragonhide/TAfireresist))
		user.remove_status_effect(/datum/status_effect/buff/dragonhide/TAfireresist)
		user.remove_status_effect(/datum/status_effect/buff/dragonhide/TAfireresist/buff)
		user.fire_stacks = 0
		devotion_cost = 0
		return TRUE
	else
		...
		devotion_cost = 30
		if(skill >= 4) //Expert++
			user.apply_status_effect(/datum/status_effect/buff/dragonhide/TAfireresist/buff)
		else
			user.apply_status_effect(/datum/status_effect/buff/dragonhide/TAfireresist)
	return TRUE
...
/datum/status_effect/buff/dragonhide/TAfireresist/proc/continue_proc()
	...
	switch(skill)
		...
		if(6)
			cost = 0
	if(user.has_status_effect(/datum/status_effect/buff/dragonhide/TAfireresist) || user.has_status_effect(/datum/status_effect/buff/dragonhide/TAfireresist/buff))
...
/datum/status_effect/buff/dragonhide/TAfireresist/buff //you can step on lava

/datum/status_effect/buff/dragonhide/TAfireresist/buff/on_apply()
	. = ..()
...
/obj/effect/proc_holder/spell/targeted/touch/summonrogueweapon/TAastratagrasp
	...
	associated_skill = /datum/skill/magic/holy
	hand_path = /obj/item/melee/touch_attack/rogueweapon/TAastratagrasp
	devotion_cost = 30
	miracle = TRUE
	range = -1
...
/obj/item/melee/touch_attack/rogueweapon/TAastratagrasp
	...
	attached_spell = /obj/effect/proc_holder/spell/targeted/touch/summonrogueweapon/TAastratagrasp
...
/datum/intent/mace/strike/TAastrata
...
/datum/intent/mace/smash/TAastrata
	hitsound = list('sound/items/firelight.ogg', 'sound/misc/frying.ogg', 'sound/misc/explode/incendiary (1).ogg', 'sound/misc/explode/incendiary (2).ogg')

/obj/item/melee/touch_attack/rogueweapon/TAastratagrasp/Initialize()
	. = ..()
	addtimer(CALLBACK(src, PROC_REF(TAskillcheck), src), wait = 1)
	ADD_TRAIT(src, TRAIT_NODROP, ABSTRACT_ITEM_TRAIT)
	item_flags |= SURGICAL_TOOL

/obj/item/melee/touch_attack/rogueweapon/TAastratagrasp/attack(mob/target, mob/living/carbon/user)
...
/obj/item/melee/touch_attack/rogueweapon/TAastratagrasp/proc/TAskillcheck()
...
/obj/item/melee/touch_attack/rogueweapon/TAastratagrasp/attack_self()
	attached_spell.remove_hand()
	qdel(src)

/obj/item/melee/touch_attack/rogueweapon/TAastratagrasp/proc/cooldown()
	cooldown = FALSE

/obj/item/melee/touch_attack/rogueweapon/TAastratagrasp/afterattack(atom/target, mob/living/carbon/user, params, proximity)
...
/obj/item/melee/touch_attack/rogueweapon/TAastratagrasp/pre_attack(atom/target, mob/living/user, params)
	if(!istype(user.a_intent, /datum/intent/use))
		return ..()
	if(isliving(target))
...
/obj/effect/proc_holder/spell/invoked/TAsunstrike/start_recharge()
	var/old_recharge = recharge_time
	var/tech_resurrection_modifier = SSchimeric_tech.get_resurrection_multiplier()
	if(tech_resurrection_modifier > 1)
		recharge_time = initial(recharge_time) * (tech_resurrection_modifier * 1.25)
...
/obj/effect/proc_holder/spell/invoked/TAsunstrike/cast(list/targets, mob/living/user)
	..()
...
/obj/effect/proc_holder/spell/invoked/TAsunstrike/cast_check(skipcharge, mob/user = usr)
	if(!..())
		return FALSE
	var/found = null
	for(var/obj/structure/fluff/psycross/S in oview(5, user))
...
/obj/effect/proc_holder/spell/self/TAastrata_sword/cast(list/targets, mob/living/user = usr)
	if(src.conjured_sword)
		qdel(conjured_sword)
	var/obj/item/rogueweapon/astrata_blade = new /obj/item/rogueweapon/sword/TAastrata_sword(user.drop_location())

	user.put_in_hands(astrata_blade)
	src.conjured_sword = astrata_blade
	return TRUE

/obj/item/rogueweapon/sword/TAastrata_sword
	name = "Solar Sabre"
	...
	force = 20			//more comparable to a dagger than a sword, for it is ultimately a tool
	force_wielded = 20
...
/obj/item/rogueweapon/sword/TAastrata_sword/proc/TAskillcheck()
	var/mob/living/carbon/human/user = src.loc
	var/skill = user.get_skill_level(/datum/skill/magic/holy)
	wdefense += skill/2
	wdefense_dynamic += skill/2
	if(skill <= 4)
		force = 5 * skill
		wdefense_wbonus += 1
	else
...
/obj/item/rogueweapon/sword/TAastrata_sword/Initialize()
	. = ..()
