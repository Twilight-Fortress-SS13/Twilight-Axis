/datum/special_intent/ranged
	abstract_type = /datum/special_intent/ranged
	use_clickloc = TRUE
...
/datum/special_intent/ranged/check_range(atom/source, atom/target)
...
/datum/special_intent/ranged/deploy(mob/living/user, atom/parent, atom/target, params)
	actual_target = target
	stored_params = params
	return ..()

/datum/special_intent/ranged/proc/prepare_shot_data()
...
/datum/special_intent/ranged/proc/setup_projectile(obj/projectile/proj, proj_range)
	if(!istype(proj))
		return
...
/datum/special_intent/ranged/proc/perform_archery_shot()
	var/obj/item/gun/ballistic/revolver/grenadelauncher/bow/B = iparent
	var/mob/living/L = howner
...
/datum/special_intent/ranged/doubleshot
    name = "Double Shot"
    desc = "Instantly looses a second arrow from your quiver right after the first. Aims for the targeted zone."
    range = 14
    use_doafter = 1 SECONDS
    stamcost = 25
    cooldown = 15 SECONDS

/datum/special_intent/ranged/doubleshot/on_create()
...
/datum/special_intent/ranged/doubleshot/proc/fire_second_arrow()
    var/obj/item/gun/ballistic/revolver/grenadelauncher/bow/B = iparent
...
/datum/special_intent/ranged/longshot
    name = "Long Shot"
    desc = "A carefully aimed, overdrawn shot. The further the target, the more devastating the impact."
    range = 25
    use_doafter = 1.5 SECONDS
    stamcost = 30
    cooldown = 20 SECONDS

/datum/special_intent/ranged/longshot/on_create()
    if(!prepare_shot_data()) return
    var/obj/item/gun/ballistic/revolver/grenadelauncher/bow/B = iparent
...
/datum/special_intent/ranged/backstep
    name = "Evasive Shot"
    desc = "Fires a hasty shot while simultaneously vaulting backward, granting a temporary burst of speed."
    range = 14
    use_doafter = 0.5 SECONDS
    stamcost = 20
    cooldown = 10 SECONDS

/datum/special_intent/ranged/backstep/on_create()
    if(!prepare_shot_data()) return
    var/obj/item/gun/ballistic/revolver/grenadelauncher/bow/B = iparent
...
/atom/movable/screen/alert/status_effect/archer_haste
    name = "Wind's Agility"
    desc = "Your movements feel significantly lighter after a swift evasive maneuver."
    icon_state = "buff"
