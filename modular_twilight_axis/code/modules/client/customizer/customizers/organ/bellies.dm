...
/datum/customizer_choice/organ/belly/validate_entry(datum/preferences/prefs, datum/customizer_entry/entry)
	...
	belly_entry.belly_size = sanitize_integer(belly_entry.belly_size, BELLY_SIZE_MIN, BELLY_SIZE_HUGE, BELLY_SIZE_MIN)
...
/datum/customizer_choice/organ/belly/handle_topic(mob/user, list/href_list, datum/preferences/prefs, datum/customizer_entry/entry, customizer_type)
	...
	switch(href_list["customizer_task"])
		if("belly_size")
			var/named_size = input(user, "Choose your belly size:", "Character Preference", find_key_by_value(BELLY_SIZES_BY_NAME, belly_entry.belly_size)) as anything in BELLY_SIZES_BY_NAME
			if(isnull(named_size))
				return
			var/new_size = BELLY_SIZES_BY_NAME[named_size]
			belly_entry.belly_size = sanitize_integer(new_size, BELLY_SIZE_MIN, BELLY_SIZE_HUGE, BELLY_SIZE_MIN)
		if("allow_to_grow")
