...
/datum/sex_session_cache_worker/proc/mark_dirty_for_tab(tab)
	switch(tab)
		if("status")
			dirty_status = TRUE
		if("actions")
...
/datum/sex_session_cache_worker/proc/tick()
	...
	if(!dirty_status && !dirty_menu && !dirty_kinks && !dirty_org_nodes && !dirty_partners && !dirty_links && !dirty_can_perform && !dirty_filtered)
		return FALSE

	var/now = world.time
	if(dirty_status && now >= next_status_time && session.active_tab == "status")
