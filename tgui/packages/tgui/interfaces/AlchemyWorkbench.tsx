...
export const AlchemyWorkbench = (props) => {
  ...
  const [tab, setTab] = useState(
    data.upgrade_lvl < 3 ? 'crafting' : 'workstation',
  );
        ...
        <Stack vertical fill>
          <Stack.Item>
            <Tabs>
              {data.upgrade_lvl >= 4 && (
                <Tabs.Tab
                  selected={tab === 'transmutation'}
                  onClick={() => setTab('transmutation')}
                >
                  Трансмутация
                </Tabs.Tab>
              )}
              ...
              <Tabs.Tab
                selected={tab === 'crafting'}
                onClick={() => setTab('crafting')}
              >
                Создание
              </Tabs.Tab>
              <Tabs.Tab
                selected={tab === 'grimoire'}
                onClick={() => setTab('grimoire')}
              >
                Гримуар
              </Tabs.Tab>
        ...
        >
          ...
          >
            <Stack align="center" justify="space-between" mb={1}>
              ...
              <Stack.Item fontSize="0.9em" color="gray">
                На 1 варку нужно 90 u воды
              </Stack.Item>
            </Stack>
            ...
            <ProgressBar
              ...
              color={
                data.reagents?.length > 0 ? data.reagents[0].color : 'blue'
              }
            >
          ...
          <Section
            ...
            buttons={
              <Button
                icon="times"
                color="transparent"
                onClick={() => setAssigningToSlot(null)}
              />
            }
          >
            {data.available_ingredients.length === 0 ? (
              <Box color="gray" italic p={1}>
                Склад лаборатории пуст. Положите ингредиенты в стол.
              </Box>
            ) : (
  ...
  return (
    <Section title="Алхимическая Кузня" fill>
      <Stack align="center" justify="center">
        ...
        >
          {data.crafting_grid.map((slot, i) => (
            <div
              ...
              onClick={() =>
                slot.ref
                  ? act('clear_craft', { slot: i + 1 })
                  : setAssigning(i + 1)
              }
            >
              {slot.image && (
                <img src={slot.image} style={{ width: 48, height: 48 }} />
              )}
            </div>
          ))}
        </div>
        ...
        <div
          style={{ width: 64, height: 64, border: '2px solid white' }}
          onClick={() => act('do_grid_craft')}
        >
          {data.craft_result && (
            <img
              src={data.craft_result.image}
        ...
        <Section title={`Выбор компонента ${assigning}`} mt={2}>
          {data.available_all_items.map((item, i) => (
            <Button
              ...
              fluid
              onClick={() => {
                act('assign_craft', { slot: assigning, item_ref: item.ref });
                setAssigning(null);
              }}
            >
              {item.name}
...
const RequirementBox = ({
  ...
  return (
    ...
    >
      <Stack align="center">
        <Box bold mr={1} color="white">
          {count}x
        </Box>
        <Icon
          name={isUnknown ? 'question-circle' : 'leaf'}
          color={color}
          mr={1}
        />
    ...
    if (reqs[tier] && reqs[tier].length > 0) {
      reqs[tier].forEach((smell) => {
        if (elements.length > 0) {
          elements.push(
            <Box key={`plus-${elements.length}`} mx={0.5} color="gray" bold>
              +
            </Box>,
          );
        }
        elements.push(
          <RequirementBox
            key={`${tier}-${smell}-${elements.length}`}
            tier={tier}
            smell={smell}
          />,
        );
        ...
        <Stack.Item>
          ...
          >
            <Stack align="center">
              ...
              <Stack.Item grow>
                <Box bold color="teal" fontSize="1.1em">
                  Накопитель Энергии
                </Box>
                <ProgressBar
                  value={data.lux_tank?.charges || 0}
                  maxValue={data.lux_tank?.max || 1000}
                  color="teal"
                  mt={0.5}
                />
                <Stack justify="space-between" mt={0.5} align="center">
                  <Box fontSize="0.9em" color="white">
                    Зарядов Люкса: {data.lux_tank?.charges || 0} /{' '}
                    {data.lux_tank?.max || 1000}
                  </Box>
                  <Button
                    icon="battery-full"
                    color="good"
                    onClick={() => act('consume_lux')}
                  >
        ...
        <Stack.Item grow>
          {data.transmute_recipes?.length > 0 ? (
            data.transmute_recipes.map((recipe, i) => {
              const hasEnergy =
                data.lux_tank && data.lux_tank.charges >= recipe.cost;
              ...
              return (
                <Button
                  ...
                  fluid
                  mb={1}
                  color={hasEnergy ? 'teal' : 'gray'}
                  disabled={!hasEnergy}
                  onClick={() =>
                    act('do_transmute', { recipe_ref: recipe.ref })
                  }
                  style={{ height: '42px' }}
                >
                  <Stack align="center">
                    ...
                    <Stack.Item grow textAlign="left">
                      <Box bold fontSize="1.1em" color="white">
                        {recipe.name}
                      </Box>
                    </Stack.Item>
                    <Stack.Item>
            ...
            {data.available_all_items.length > 0 ? (
              data.available_all_items.map((item, i) => (
                <Button
                  ...
                  mb={1}
                  onClick={() => {
                    act('transmute_assign', { item_ref: item.ref });
                    setSelecting(false);
                  }}
                >
                  <Stack align="center">
              ...
              <Box italic color="gray" textAlign="center">
                Склад стола пуст...
              </Box>
            )}
          </Section>
    ...
    >
      <Stack align="center">
        ...
        <Stack.Item grow ml={1}>
          {beaker ? (
            <Box>
              <Box bold fontSize="0.9em" color="blue">
                {beaker.cur} / {beaker.max} u
              </Box>
              <Stack mt={0.5}>
                <Stack.Item grow>
                  <Button
                    ...
                    color="good"
                    onClick={() =>
                      setTransferMenu({ slot: num, mode: 'to_cauldron' })
                    }
                  >
                    В Котел
                  </Button>
                ...
                <Stack.Item grow ml={0.5}>
                  <Button
                    ...
                    color="average"
                    onClick={() =>
                      setTransferMenu({ slot: num, mode: 'from_cauldron' })
                    }
                  >
                    Из Котла
                  </Button>
              ...
              <Button
                ...
                mt={0.5}
                color="warning"
                onClick={() => setTransferMenu({ slot: num, mode: 'to_pills' })}
              >
                В пыль
              </Button>
            </Box>
    ...
    <Stack vertical fill>
      <Stack.Item>
        <Section title="Манипуляция емкостями">
          <Stack fill>
            <Stack.Item grow>
              <BeakerSlot num="1" beaker={data.beaker1} />
            </Stack.Item>
            <Stack.Item grow ml={1}>
              <BeakerSlot num="2" beaker={data.beaker2} />
            </Stack.Item>
          </Stack>
        </Section>
      </Stack.Item>
          ...
          <Stack fill align="center">
            <Stack.Item>
              ...
              >
                ...
                <Box fontSize="1.5em" color="silver" bold>
                  {data.flour_count}
                </Box>
                <Box fontSize="0.7em" color="gray">
                  (на складе)
            ...
            <Stack.Item grow ml={2}>
              <Stack justify="space-between" mb={0.5}>
                <Box bold color="teal">
                  Буфер смеси: {data.pill_mix_total} u
                </Box>
                <Button
                  icon="reply-all"
                  color="transparent"
              ...
              >
                {data.pill_mix?.map((r, i) => (
                  <div key={i} style={{ borderBottom: '1px solid #222' }}>
                    ...
                    <b style={{ color: '#00ffcc' }}>{r.vol}u</b>
                  </div>
                ))}
          ...
          <Section
            ...
            buttons={
              <Button
                icon="times"
                color="transparent"
                onClick={() => setPillDrainMenu(false)}
              />
            }
          >
            {data.pill_mix?.map((r, i) => (
              ...
              >
                ...
                <Stack.Item>
                  <Box mb={0.5} textAlign="right">
                    <Box inline mr={1} fontSize="0.8em" color="gray">
                      В Котел:
                    </Box>
                    <Button
                      onClick={() => {
                        act('pill_to_cauldron', { reagent: r.id, amount: 5 });
                        setPillDrainMenu(false);
                      }}
                    >
                      5
                    </Button>
                    <Button
                      ml={0.5}
                      onClick={() => {
                        act('pill_to_cauldron', { reagent: r.id, amount: 15 });
                        setPillDrainMenu(false);
                      }}
                    >
                      15
        ...
        >
          ...
          >
            {(transferMenu.mode === 'from_cauldron'
              ...
              : transferMenu.slot === '1'
                ? data.beaker1?.contents
                : data.beaker2?.contents
            )?.map((r, i) => (
              <Stack
                key={i}
                mb={1}
              ...
              >
                ...
                <Stack.Item>
                  <Button
                    onClick={() => {
                      const action =
                        transferMenu.mode === 'to_cauldron'
                          ? 'pour_to_cauldron'
                          : transferMenu.mode === 'from_cauldron'
                            ? 'fill_from_cauldron'
                            : 'add_to_pill_mix';
                      act(action, {
                        slot: transferMenu.slot,
                        reagent: r.id,
                        amount: 5,
                      });
          ...
          <Section
            ...
            buttons={
              <Button
                icon="times"
                color="transparent"
                onClick={() => setSelectingBeaker(null)}
              />
            }
          >
            {data.available_all_items.length > 0 ? (
              data.available_all_items.map((item, i) => (
                <Button
                  ...
                  mb={1}
                  onClick={() => {
                    act('lab_assign_beaker', {
                      slot: selectingBeaker,
                      item_ref: item.ref,
                    });
                    setSelectingBeaker(null);
                  }}
                >
                  <Stack align="center">
                    <img
  ...
  return (
    <Section
      ...
      buttons={
        <Tabs>
          <Tabs.Tab
            selected={subTab === 'potions'}
            onClick={() => setSubTab('potions')}
          >
            Зелья
          </Tabs.Tab>
          <Tabs.Tab
            selected={subTab === 'crafting'}
            onClick={() => setSubTab('crafting')}
          >
            Инструменты
          </Tabs.Tab>
        </Tabs>
    ...
    >
      <Stack vertical fill scrollable>
        {filteredKnowledge.length === 0 && (
          <Box italic color="gray" textAlign="center" mt={5}>
            Записи пустуют...
          </Box>
        )}
              ...
              >
                <Stack align="center" justify="space-between">
                  ...
                  <Stack.Item>
                    <Stack align="center">
                      {isInfusion && (
                        <InfusionRequirements reqs={recipe.reqs} />
                      )}
                      {isGrid && (
                        <GridPreview icons={recipe.grid_icons || []} />
                      )}
